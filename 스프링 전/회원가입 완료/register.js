...
function nick_use() {
	...
	opener.document.getElementById("user_nick").disabled = true;
	window.close();
}
...
function id_use() {
	...
	opener.document.getElementById("user_id").disabled = true;
	window.close();
}
...
function register_ok() {
	...
	if(document.getElementById("comCheckBtn").checked == true) {
		if(document.register_form.com_name.value.length == 0) {
			alert("기업으로 체크 시 반드시 기업명을 입력해야합니다.\n만약 기업이 아니라면 기업 체크를 해제해주세요.");
			document.register_form.com_name.focus();
			return;
		}
	}
