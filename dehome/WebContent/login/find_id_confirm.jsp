...
<%
	...
	String find_id = dao.findID(name, email, email2);	
%>
<!DOCTYPE html>
<html>
<head>
...
%>
				<script type="text/javascript">
					...
					location.href='../login/find_id.jsp';
				</script>
<%
			} else {
%>
				<p class="id_info">회원님의 아이디는 <%= find_id %>입니다.</p>
<%
			}
%>
				<div class="id_info_btn_div">
					<input type="button" value="로그인하기" onclick="location.href='../login/login.jsp'" class="id_info_btn"/>
					<input type="button" value="비밀번호 찾기" onclick="location.href='../login/find_pw.jsp'" class="id_info_btn"/>
				</div>
			</div>
		</div>
	</form>
