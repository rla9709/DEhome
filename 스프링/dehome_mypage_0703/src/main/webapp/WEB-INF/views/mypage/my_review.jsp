...
<%--나중에,,,,,,,,,,,,,,,,,,,, --%>
      ...
      <div id="review_content">
		 <c:forEach items="${myReview }" var="myReview" end="3" varStatus="status">
        ...
        <script type="text/javascript">
			var more = -1;
			$(function(){$('#moreList').on('click',function(){
				var path = document.getElementById("path").value;
				more = more + 1
				console.log(more);
					$.ajax({
						url : "my_review.moreList",
						type : "post",
						data: { more : more},	
						dataType: "json",
						success: function(data) {
							...
							for(var i in data.myReview) {
								if( i % 2 == 0 ){
									...
									html+= "<p>";
									html+= "<span>"+data.myReview[i].re_title+"</span>";
									html+= "<span>"+data.myReview[i].com_name+"</span>";
									html+= "<img src='"+ path +"/resources/images/star_yellow_15.png' /> ";
									html+= "<span>"+data.myReview[i].com_rating +"</span>";
									html+= "</p>";
									html+= "<p>"+ data.myReview[i].re_content;
									html+= "</p>";
									html+= "</div>";
									html+= "</a>";
								}else{
