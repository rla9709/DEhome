...
/*--시공상담 채팅--*/
	...
	public ModelAndView goCounChatting(HttpServletRequest request, InteriorDTO dto, HttpSession session, ChatDTO dto3) throws Exception {
		...
		String sessionNick = (String) session.getAttribute("nickname");
		System.out.println("세션닉 : "+ sessionNick);
		...
		if (user_nick_com == null || user_nick_com == "" || user_nick_com.isEmpty()) {
			dto3.setUser_nick(sessionNick);
			user_nick_com = "notCom";
			System.out.println("user_nick_com : "+ user_nick_com);
		} else {
			dto3.setUser_nick(user_nick_com);
		}
		dto3.setCom_name(com_name);
		
		String nick = service.getNickCoun(dto3);
		if(request.getParameter("nick2") != null) {
			nick = request.getParameter("nick2");
		...
		ModelAndView mav = new ModelAndView();
		mav.addObject("nick", nick);
		mav.addObject("user_nick_com", user_nick_com);
		mav.addObject("dto" , dto2);
	...
	public void insertChatCoun(HttpServletRequest request, HttpServletResponse response, ChatDTO dto) throws Exception {
		...
		if(user_nick == null || user_nick.equals("") || to_nick == null || to_nick.equals("") 
			...
			if (user_nick_com != null) { // 해당기업이면 user_nick(보내는 사람)에 기업명 넣기
				user_nick = user_nick_com;
			}
			
			dto.setUser_nick(user_nick);
			//업체가 로그인했을경우 to_nick에 상대방 유저를 대신넣어줌
			if(user_nick.equals(to_nick)) {
				dto.setTo_nick(nick);
			} else {
				dto.setTo_nick(to_nick);
	...
	public void listChatCoun(HttpServletRequest request, HttpServletResponse response, ChatDTO dto) throws Exception {
		...
		String user_nick = request.getParameter("user_nick");
		String to_nick = request.getParameter("to_nick");
		String chat_nick = request.getParameter("chat_nick");
		String listType = request.getParameter("listType");
		String nick = request.getParameter("nick");
		String com_name = request.getParameter("com_name");
		...
		if(user_nick == null || user_nick.equals("") || to_nick == null || to_nick.equals("") 
			...
			response.getWriter().write(getTenCoun(URLDecoder.decode(user_nick,"utf-8"), URLDecoder.decode(to_nick,"utf-8"), URLDecoder.decode(nick,"utf-8"), 100, com_name));
		} else {
			try {
				response.getWriter().write(getIDCoun(URLDecoder.decode(user_nick,"utf-8"), URLDecoder.decode(to_nick,"utf-8"), URLDecoder.decode(nick,"utf-8"), listType, com_name));
			} catch (Exception e) {
				response.getWriter().write("");
			}
		}
	...
	//최대 100개까지 출력하게끔함
	public String getTenCoun(String user_nick, String to_nick,String nick, int number, String com_name) throws Exception {
		ChatDTO dto = new ChatDTO();
		...
		if(user_nick.equals(to_nick)) { // user_nick(보내는 사람) == to_nick(기업명)
			dto.setTo_nick(nick); // to_nick(받는 사람)에 상담중인 유저 넣기
		} else {
			dto.setTo_nick(to_nick);
		}
	...
	public String getIDCoun(String user_nick, String to_nick, String nick, String list_type, String com_name) throws Exception {
		ChatDTO dto = new ChatDTO();
		dto.setUser_nick(user_nick);
	...
	public ModelAndView goCounChattingList(HttpServletRequest request, HttpSession session, ChatDTO dto) throws Exception {
		...
		if (user_nick_com != null) { // 기업이면 유저닉에 기업명 넣기
			dto.setUser_nick(user_nick_com);
		} else {
			dto.setUser_nick(user_nick);
		}
		...
		if (user_nick_com != null) { // 기업이면
			System.out.println("기업보내기");
			list = service.getChatListCounCom(dto);
		} else {
			System.out.println("유저보내기");
			list = service.getChatListCounUser(dto);
		}
