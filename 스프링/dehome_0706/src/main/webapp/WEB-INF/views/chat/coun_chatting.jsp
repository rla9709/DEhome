...
<c:set var="nick2"  value="${param.nick2}"/>
...
console.log( 'sadfsaf__>>>'+ ${dto.com_name})
// 채팅 입력ajax
var lastID = 0;
function insertFunction(){
	var user_nick = '<%=sessionNick%>';
	var to_nick = '${dto.com_name}';
	var nick = '${nick}';
	var com_name = '${dto.com_name}';
...
function chatListFunction(type) {
	...
	$.ajax({
		type: 'POST',
		url: '../chatListCoun.do?com_name=${dto.com_name}',
		data: {
			user_nick: encodeURIComponent(user_nick),
			to_nick: encodeURIComponent(to_nick),
			nick: encodeURIComponent(nick),
...
<%@include file ="../headerfooter/header.jsp" %>
	...
	<div id="chatting">
		<div id="chatting_title">
			...
			<p>
				<c:choose>
					<c:when test="${nick2 eq nickname}">
						<span class="font_green">${dto.com_name}</span> 님과 대화중입니다
					</c:when>
					<c:otherwise>
						<span class="font_green">${nick}</span> 님과 대화중입니다
					</c:otherwise>
				</c:choose>
			</p>
		</div>
