...
<body>
    ...
    <main>
      <form name="reg_goods_form" action="../trade/used_trade_edit" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
      	<input type="hidden" name="tr_no" value="${dto.tr_no}">
        <div class="sell_form">
          <p class="regi_form_main_title">내 글 수정</p>
          <div>
          ...
          <div class="goods_regi_form regi_form_title">
            <input type="text" name="tr_title" maxlength="20" placeholder="글 제목" value="${dto.tr_title}" />
          </div>
          <div class="goods_regi_form regi_form_price">
            <c:choose>
            	<c:when test="${dto.tr_price eq '무료나눔'}">
		            <select name="price_category" id="price_category" onchange="select_change()">
		              <option value="1">판매금액</option>
		              <option value="2" selected>무료나눔</option>
		            </select>
            	</c:when>
            	<c:otherwise>
            		<select name="price_category" id="price_category" onchange="select_change()">
		      ...
		      <input type="text" maxlength="10" placeholder="숫자만 입력(0원일 경우 무료나눔을 선택해주세요.)" id="price" name="tr_price" value="${dto.tr_price}" />
			 </div>
          <div class="goods_regi_form_file">
            ...
            <input
              ...
              accept="image/*"
            />
            <input type="text" id="fileName" name="fileName" readonly="readonly" value="${dto.tr_photo}" />
          </div>
            <div class="select_img_edit"><img src="${path}${dto.tr_photo}" /></div>
			...
			$("#ex_file").change(function(){
				if(this.files && this.files[0]) {
				    ...
				    reader.onload = function(data) {
				    	$(".select_img_edit").attr("src", data.target.result).css('display','none');        
          ...
          >${dto.tr_content}</textarea>
          <div class="goods_submit_btn">
            <input type="reset" value="새로고침" id="reset"/>
	        <input type="button" value="정보수정" onclick="check_ok()"/> -
          </div>
        </div>
      </form>
    </main>
